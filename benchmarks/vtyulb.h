...
namespace LFStructs {
    ...
    public:
        ...
        FastSharedPtr(FastSharedPtr<T> &&other)
                : knownValue(other.knownValue), foreignPackedPtr(other.foreignPackedPtr), data(other.data) {
    ...
    private:
        ...
        FastSharedPtr(std::atomic<size_t> *packedPtr)
                : knownValue(packedPtr->fetch_add(1) + 1), foreignPackedPtr(packedPtr), data(getControlBlock()->data) {
